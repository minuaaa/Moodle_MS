 ...
 * Helper class to enqueue fonts.
 *
 * Class eLearning_Generate_Fonts
 */
class eLearning_Generate_Fonts {
	...
	public static function get_fonts() {
		 ...
		 * @since   1.0.0
		 */
		return apply_filters( 'elearning_add_fonts', self::$fonts );
	...
	public static function render_fonts() {
		 ...
		 * @since   1.0.0
		 */
		$font_list = apply_filters( 'elearning_render_fonts', self::get_fonts() );

		$google_fonts = array();
		$font_subset  = array();

		$system_fonts = eLearning_Fonts::get_system_fonts();
		...
		foreach ( $font_list as $name => $font ) {
			...
			if ( ! empty( $name ) && ! isset( $system_fonts[ $name ] ) ) {
				 ...
				 * @since   1.0.0
				 */
				$subset = apply_filters( 'elearning_font_subset', '', $name );
				if ( ! empty( $subset ) ) {
		...
		$host_fonts_locally = get_theme_mod( 'elearning_load_google_fonts_locally', 0 );

		if ( 1 == $host_fonts_locally ) {
			wp_enqueue_style( 'elearning_googlefonts', elearning_get_webfont_url( 'https:' . $google_font_url ), array(), ELEARNING_THEME_VERSION, 'all' );
		} else {
			wp_enqueue_style( 'elearning_googlefonts', $google_font_url, array(), ELEARNING_THEME_VERSION, 'all' );
		}
	...
	public static function google_fonts_url( $fonts, $subsets = array() ) {
		 ...
		 * @since   1.0.0
		 */
		$fonts = apply_filters( 'elearning_google_fonts_selected', $fonts );
